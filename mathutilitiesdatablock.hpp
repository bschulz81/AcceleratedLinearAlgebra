...
#ifndef DATABLOCKUTILITIEShpp
#define DATABLOCKUTILITIEShpp
...
#pragma omp begin declare target
 template<typename T>
inline bool DataBlockUtilities::same_extents(const DataBlock<T>&d1,const DataBlock<T>&d2)
{
...
{
    if(d1.dpextents[i]!=d2.dpextents[i])
        return false;
}
return true;
...
void DataBlockUtilities::copy(DataBlock<T>&target,const DataBlock<T>& source)
{
    ...
    target.dpconjugate=source.dpconjugate;
