...
In general, OpenMP would be preferable for a scientific library, because in OpenMP one has nested parallelism, where parallel for loops can call functions which open other parallel for loops. This is essential if one has complex algorithms where one may have procedures like solving eigenvalues which contain parallel loops, within another parallel loop...
