...
With block cyclic ordering and a class for distributed tensors that allowes distriuting data for any mpi network size, it is now much easier to implement load balanced parallel tensor and matrix algorithms that use several mpi nodes. By now only the Strassen and Winograd variant of the Strassen algorithm are implemented. With a fast OpenMP, MPI async optimized scatterer that uses flexible scattering grids, and a dedicated distributed Tensor class that stores the necessary metadata and local data, distributing data over severa nodes is now convenient for cpu and gpu with cuda aware OpenMpi. 
