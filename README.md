...
Gcc is mathematically correct since infinity times 0 can be anything. With the fix, the execution of the Kernels now works as before the coefficients were added.

Added first tensor kernels for higher ranks, but they are currently untested so far.

I added a flat index for parallelization. The (i) operator of the datablock class for vectors now checks the rank. 
For rank==1 it works as usual but for higher ranks, (i) is assumed to be a flat tensor index whose data offset it computes in a while loop that can even be used within simd lanes.

This should allow parallelization of tensor operations with higher ranks in a single loop once the maximum index is calculated. But this is currently entirely untested...
...
Removed the old various copy functions for subtensors and made the deep copy mechanism of views more flexible. One can now designate the device or a memory map.
...
But I haven't tested this as it is not yet available on gentoo.

Clang does not show this problem and can reduce complex variables on target devices in test examples. 
...
The scatterer and gatherer are optimized with OpenMP and asynchroneous MPI send and recieve. The test application now demonstrates tensor scattering. Outcommented sections demonstrate matrix scattering if activated, and can be easily replaced with row/column scattering. A DistributedDatablock class was added which can store distributed tensors that sit on several MPI nodes. This class can recieve multiple DataBlocks per mpi node which makes the number of mpi nodes irrelevant for the success of the scattering and gathering operations. Bugs in the memory routines were fixed... 

With block cyclic ordering and a class for distributed tensors that allowes distriuting data for any mpi network size, it is now much easier to implement load balanced parallel tensor and matrix algorithms that use several mpi nodes. By now only the Strassen and Winograd variant of the Strassen algorithm are implemented. With a fast OpenMP, MPI async optimized scatter and gather functions that uses flexible scattering grids to distribute tensors, and a distributed Tensor class that stores several blocks of local data, distributing data over several nodes is now convenient for cpu and gpu with cuda aware OpenMpi. 

### 04.03.2026
Fixed an indexing bug in datablock for the parallel copy of a column of a subspan. Changed the cmakelists.txt to support clang 22. Changed the tutorial at https://forums.gentoo.org/viewtopic-t-1172069.html how to compile gcc and clang 22 for gpu offload on gentoo Linux
...
Even if the library appears to work around the disastrous gcc bug https://gcc.gnu.org/bugzilla/show_bug.cgi?id=123597, I can currently not recommend gcc versions earlier than the gcc-16 development version where this is now fixed, until the patch https://gcc.gnu.org/git/?p=gcc.git;a=commit;h=1ae53b20c2474c28da13835719cceeee0702c966 is backported to earlier gcc versions. 

The compiler bug, which I discovered, lets gcc move variables of template type outside of their scopes, leading to wrong numerical results in many OpenMP applications which can be rare enough (10%) not to be detected easily in large computations. 
...
mkdir /etc/portage/patches/sys-cluster/openmpi-5.0.9/

and put the patch into this directory. Then

emerge --sync;
emerge =sys-cluster/openmpi-5.0.9;
...
The clang compiler in version 21.1.7 compiles the code of this library and the test programs correctly. the test programs run and compile with zero errors in cuda-sanitizer, apart from the arraytest_mpi program that uses the open message passing interface OpenMPI.

Unfortunately, there is currently a general conflict with the cuda runtime initialization between clang when configured for nvptx offload and cuda-aware OpenMPI https://github.com/open-mpi/ompi/issues/13431#issue-3501189152. To address this, either clang or, in that case more likely, OpenMpi has to adapt their code. The issues seems to be that clang, if configured for offload, automatically reserves cuda memory on startup which OpenMPI also tries to reserve at initialization. This leads to an error message at startup, but the program arraytest_mpi nevertheless seems to execute over mpi and computes the correct results.
...
### 06.09.25:
Fixed a bug in the gpu version of the advanced algorithms for the Cholesky decomposition. All the Algorithms from https://arxiv.org/pdf/1812.02056 as well as the Strassen Algorithm and Its Winograd variant https://arxiv.org/abs/1410.1599 now work on device.
