...
The scatter and gather functions are designed such that they also work if the number of nodes is larger than the problem. Say you have a matrix which you scatter into 200 rows but your cluster has 2000 nodes. In that case, only 200 ranks would recieve data, the others nullpointers. The gather functions are such that they can acommodate and reject the nullpointers when building the tensors from the scattered subsets.
